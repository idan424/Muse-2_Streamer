...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            print('channel data', ch_data)
            print("raw data", data_epoch)
            # Compute band powers
            band_powers = utils.compute_band_powers(data_epoch, fs)
            band_buffer, _ = utils.update_buffer(band_buffer, np.asarray([band_powers]))

            # Compute the average band powers for all epochs in buffer - This helps to smooth out noise
            ...
            # this is the hackathon code ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            '''
            alphasum += alpha_metric
            current_iteration += 1
            
            if current_iteration == 10:
                average_alpha = alphasum / 10
                current_iteration = 0
                alphasum = 0
                print("avg alpha: "+str(average_alpha))
                if(average_alpha > 1):
                    showinfo("Please focus!", "You should focus on your job!")
            '''
